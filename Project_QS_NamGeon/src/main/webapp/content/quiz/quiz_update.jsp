...
<%
    ...
    if(start !=null){
	 	...
	 	Content_List_DTO my_cl = cl_dao.getContentByKey(content_key);
	 	ArrayList<Quiz_List_DTO> my_lists = ql_dao.getQuizByKey(content_key);
	 	content_quiz.settingContent(my_cl,my_lists);
	 	response.sendRedirect("quiz_update.jsp?content_key="+content_key);
    }
...
<script type="text/javascript">
    ...
    function delete_quiz_list_update(number){
        location.href = "quiz_list_delete.jsp?number="+number+"&content_key="+content_key;
    }
</script>
<div class="create_modal_bg"></div>
<div class="create_modal_wrap p-3">
    <div class="content_info_area pt-2 pb-4 px-3 text-center ">
        <form action="quiz_info_add.jsp?member_key=<%=member_key%>&content_key=<%=content_key %>" class="text-start p-3 pb-1 mb-0" method="post"  enctype="multipart/form-data">
            <p class="">퀴즈만들기</p>
            <hr>
...
<div class="create_quiz_modal_wrap p-3">
    ...
    <div class="container" id="container">
        <div id="content" class="row g-5 quiz_content">
        		...
        		for(int i=0;i<lists.size();i++){
        			...
        			%>
			            <div class="col-xl-3 col-lg-4 col-sm-6 col-12">
			                <div class="quiz_content_info_box p-4" onclick="update_quiz_list_update(<%=i%>,<%=type%>,'<%=question%>','<%=answer%>','<%=example%>','<%=image%>',<%=content_key%>)">
                                ...
                                <i class="bi bi-trash3-fill btn" id="quiz_list_delete" onclick="delete_quiz_list_update(<%=i%>,<%=content_key%>)"></i>
			                </div>
			            </div>
        			<%
        		}
            ...
            %>
            <div class="col-xl-3 col-lg-4 col-sm-6 col-12">
                <div class="create_quiz_btn text-center p-3" onclick="create_quiz_list_update('<%=last_question%>',<%=last_type%>,<%=content_key%>)">
    ...
    <div id="quiz_controller">
        <div class="controller_btn_box">
            <button id="info_controller_btn" onclick="info_controller_update('<%=title%>','<%=explanation%>','<%=thumbnail%>','<%=content_public%>',<%=content_key%>)"><i class="bi bi-info-lg"></i></button>
            <button id="insert_controller_btn" onclick="insert_controller_update(<%=lists.size()%>,<%=content_key%>)"><i class="bi bi-floppy"></i></button>
            <button id="delete_controller_btn" onclick="delete_controller_update()"><i class="bi bi-arrow-return-left"></i></button>
        </div>
    </div>
